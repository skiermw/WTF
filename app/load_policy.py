...
def CreateQuote(pol_json):
    ...
    print('  Policy stream ID: %s' % policy_stream_id)
    print('  Policy eff date: %s' % response_json['effectiveTimestamp'])
    outfile.write(policy_stream_id +","+response_json['effectiveTimestamp']+"\n")
    

def main():
    # This program expects 2 varibles to be passed on the commandline:
    #     environment ("devl" or "test" or "demo")
    #     input file of policies to load (json)
    
    global server
    global filename
    
    if (len(sys.argv)< 3 ): 
        print("Usage is: load_policy.py <devl | demo | test> <filename>")
        sys.exit(0)
    else:
        if sys.argv[1] == "devl":
            server = "dcdevappsrv1"
